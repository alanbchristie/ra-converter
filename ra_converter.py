...
_RA_TARGET = input("RA target coordinate (H:M): ")
assert _RA_TARGET
# Check the value looks sensible..
# (i.e. represents a clock)
_PARTS: List[str] = _RA_TARGET.split(':')
assert len(_PARTS) == 2
_TARGET_H: int = int(_PARTS[0])
assert 0 <= _TARGET_H < 24
_TARGET_M: int = int(_PARTS[1])
...
print('Local time (UTC): %s' % str(_MIDNIGHT_OFFSET)[:-3])
print('Adjusted RA coordinate: %s' % str(_ADJUSTED_RA)[:-3])
